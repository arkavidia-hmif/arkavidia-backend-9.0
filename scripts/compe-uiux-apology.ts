...
const sentEmailFilePath = 'scripts/email-targets/compe-uiux-emails-sent.csv';
...
async function main() {
  try {
    ...
    const emailFile = fs.readFileSync(
      'scripts/email-targets/compe-uiux-emails.csv',
      'utf8',
    );
      ...
      unsentEmails.map(async (val) => {
        try {
          ...
          const emailTemplatePath =
            'scripts/email/compe-uiux-apology.template.html';
          sentEmail = await transporter.sendMail({
            from: MAIL_FROM,
            to: val.email,
            subject: `[Arkavidia ${val.title} 9.0] There is no groupchat!`,
